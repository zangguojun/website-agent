...
import { runClarifyPhaseStream } from '../../../../../../streaming/phase-streams';
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const phaseBlocked = assertStreamPhase(gate.session, 'clarify');
    if (phaseBlocked) return phaseBlocked;

    return runClarifyPhaseStream(gate.session, ownerId);
  } catch (error) {
