...
import { runQuestionsPhaseStream } from '../../../../../../streaming/phase-streams';
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const phaseBlocked = assertStreamPhase(gate.session, 'questions');
    if (phaseBlocked) return phaseBlocked;

    return runQuestionsPhaseStream(gate.session, ownerId);
  } catch (error) {
