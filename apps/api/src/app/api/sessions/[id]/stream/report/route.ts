...
import { runReportPhaseStream } from '../../../../../../streaming/phase-streams';
...
export async function GET(request: Request, context: RouteContext) {
  try {
    ...
    const phaseBlocked = assertStreamPhase(gate.session, 'report');
    if (phaseBlocked) return phaseBlocked;

    return runReportPhaseStream(gate.session, ownerId);
  } catch (error) {
